...
def normalizar_nombre_entidad_no_labora(df: pd.DataFrame) -> pd.DataFrame:
    if not {"situacion_laboral", "nombre_entidad"}.issubset(df.columns):
        return df

    asignar_texto(
        df,
        sit(df) == "No labora actualmente",
        "nombre_entidad",
        "NO CORRESPONDE",
    )
    return df
